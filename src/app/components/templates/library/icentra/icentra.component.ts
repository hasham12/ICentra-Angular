...
@Component({
  selector: 'app-icentra',
  templateUrl: './icentra.component.html',
  styleUrls: ['./icentra.component.css']
})
export class IcentraComponent implements OnInit {
  /*****ngxfileupload */
  ...
  constructor(
    ...
    /****ngxupload */
    this.options = { concurrency: 1, maxUploads: 50 };
    this.files = [];
...
onUploadOutput(output: UploadOutput): void {
  ...
  if (output.type === 'allAddedToQueue') {
    const event: UploadInput = {
      ...
      data: { category_id: category_id, user_id: user_id, library_id: this.library_id, from: 'icentra' }
    };
...
deleteFile(file_id) {
  ...
  if (file_id) {
    ...
    { file_id: file_id, user_id: this.AuthUser.id}
    ).subscribe(
      res => {
...
filterFiles() {
  ...
  downloadFile(path) {
    window.location.href = this.Api.baseUrl + '/library/download?path=' + path;
  }
